"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
var axios_1 = require("axios");
var PagueloFacil = /** @class */ (function () {
    /**
     *
     * @params
     * cclw: string
     * @params
     * token: string
     *
     * @optional
     * environment: development | production
     *
     */
    function PagueloFacil(cclw, token, environment) {
        if (environment === void 0) { environment = 'development'; }
        this.cclw = cclw;
        this.url = environment === 'development' ? 'https://sandbox.paguelofacil.com' : 'https://secure.paguelofacil.com';
        this.header = { authorization: token, "content-type": 'application/json' };
    }
    /**
     *
     * @param paymentInformation
     * @param card
     *
     * @returns ServerResponse
     */
    PagueloFacil.prototype.Authorization = function (paymentInformation, card) {
        var _this = this;
        var body = {
            cclw: this.cclw,
            amount: paymentInformation.amount,
            taxAmount: paymentInformation.taxAmount,
            email: paymentInformation.email,
            phone: paymentInformation.phone,
            concept: paymentInformation.concept,
            description: paymentInformation.description,
            lang: paymentInformation.lang ? paymentInformation.lang : 'ES',
            cardInformation: card
        };
        if (paymentInformation.customFieldValues && paymentInformation.customFieldValues.length > 0) {
            body.customFieldValues = paymentInformation.customFieldValues;
        }
        return new Promise(function (resolve) { return __awaiter(_this, void 0, void 0, function () {
            var response, error_1;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 2, , 3]);
                        return [4 /*yield*/, axios_1.default.post(this.url + "/rest/processTx/AUTH", body, { headers: this.header })];
                    case 1:
                        response = _a.sent();
                        resolve(response.data);
                        return [3 /*break*/, 3];
                    case 2:
                        error_1 = _a.sent();
                        if (error_1)
                            throw new Error(error_1);
                        return [3 /*break*/, 3];
                    case 3: return [2 /*return*/];
                }
            });
        }); });
    };
    /**
     *
     * @param reverseInformation
     *
     * @returns ServerResponse
     */
    PagueloFacil.prototype.ReverseAuthorization = function (reverseInformation) {
        var _this = this;
        var body = {
            cclw: this.cclw,
            codOper: reverseInformation.codOper,
            amount: reverseInformation.amount,
            description: reverseInformation.description,
            lang: reverseInformation.lang ? reverseInformation.lang : 'ES',
        };
        return new Promise(function (resolve) { return __awaiter(_this, void 0, void 0, function () {
            var response, error_2;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 2, , 3]);
                        return [4 /*yield*/, axios_1.default.post(this.url + "/rest/processTx/REVERSE_AUTH", body, { headers: this.header })];
                    case 1:
                        response = _a.sent();
                        resolve(response.data);
                        return [3 /*break*/, 3];
                    case 2:
                        error_2 = _a.sent();
                        if (error_2)
                            throw new Error(error_2);
                        return [3 /*break*/, 3];
                    case 3: return [2 /*return*/];
                }
            });
        }); });
    };
    /**
     *
     * @param captureInformation
     * @param card
     *
     * @returns ServerResponse
     */
    PagueloFacil.prototype.Capture = function (captureInformation) {
        var _this = this;
        var body = {
            cclw: this.cclw,
            amount: captureInformation.amount,
            taxAmount: captureInformation.taxAmount,
            email: captureInformation.email,
            phone: captureInformation.phone,
            concept: captureInformation.concept,
            description: captureInformation.description,
            lang: captureInformation.lang ? captureInformation.lang : 'ES',
            codOper: captureInformation.codOper
        };
        if (captureInformation.customFieldValues && captureInformation.customFieldValues.length > 0) {
            body.customFieldValues = captureInformation.customFieldValues;
        }
        return new Promise(function (resolve) { return __awaiter(_this, void 0, void 0, function () {
            var response, error_3;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 2, , 3]);
                        return [4 /*yield*/, axios_1.default.post(this.url + "/rest/processTx/CAPTURE", body, { headers: this.header })];
                    case 1:
                        response = _a.sent();
                        resolve(response.data);
                        return [3 /*break*/, 3];
                    case 2:
                        error_3 = _a.sent();
                        if (error_3)
                            throw new Error(error_3);
                        return [3 /*break*/, 3];
                    case 3: return [2 /*return*/];
                }
            });
        }); });
    };
    /**
     *
     * @param reverseInformation
     *
     * @returns ServerResponse
     */
    PagueloFacil.prototype.ReverseCapture = function (reverseInformation) {
        var _this = this;
        var body = {
            cclw: this.cclw,
            codOper: reverseInformation.codOper,
            amount: reverseInformation.amount,
            description: reverseInformation.description,
            lang: reverseInformation.lang ? reverseInformation.lang : 'ES',
        };
        return new Promise(function (resolve) { return __awaiter(_this, void 0, void 0, function () {
            var response, error_4;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 2, , 3]);
                        return [4 /*yield*/, axios_1.default.post(this.url + "/rest/processTx/REVERSE_CAPTURE", body, { headers: this.header })];
                    case 1:
                        response = _a.sent();
                        resolve(response.data);
                        return [3 /*break*/, 3];
                    case 2:
                        error_4 = _a.sent();
                        if (error_4)
                            throw new Error(error_4);
                        return [3 /*break*/, 3];
                    case 3: return [2 /*return*/];
                }
            });
        }); });
    };
    /**
     *
     * @param paymentInformation
     * @param card
     *
     * @returns ServerResponse
     */
    PagueloFacil.prototype.AuthCapture = function (paymentInformation, card) {
        var _this = this;
        var body = {
            cclw: this.cclw,
            amount: paymentInformation.amount,
            taxAmount: paymentInformation.taxAmount,
            email: paymentInformation.email,
            phone: paymentInformation.phone,
            concept: paymentInformation.concept,
            description: paymentInformation.description,
            lang: paymentInformation.lang ? paymentInformation.lang : 'ES',
            cardInformation: card
        };
        if (paymentInformation.customFieldValues && paymentInformation.customFieldValues.length > 0) {
            body.customFieldValues = paymentInformation.customFieldValues;
        }
        return new Promise(function (resolve) { return __awaiter(_this, void 0, void 0, function () {
            var response, error_5;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 2, , 3]);
                        return [4 /*yield*/, axios_1.default.post(this.url + "/rest/processTx/AUTH_CAPTURE", body, { headers: this.header })];
                    case 1:
                        response = _a.sent();
                        resolve(response.data);
                        return [3 /*break*/, 3];
                    case 2:
                        error_5 = _a.sent();
                        if (error_5)
                            throw new Error(error_5);
                        return [3 /*break*/, 3];
                    case 3: return [2 /*return*/];
                }
            });
        }); });
    };
    /**
     *
     * @param recurrentInformation
     *
     * @returns ServerResponse
     */
    PagueloFacil.prototype.Recurrent = function (recurrentInformation) {
        var _this = this;
        var body = {
            cclw: this.cclw,
            amount: recurrentInformation.amount,
            taxAmount: recurrentInformation.taxAmount,
            email: recurrentInformation.email,
            phone: recurrentInformation.phone,
            concept: recurrentInformation.concept,
            description: recurrentInformation.description,
            lang: recurrentInformation.lang ? recurrentInformation.lang : 'ES',
            codOper: recurrentInformation.codOper
        };
        if (recurrentInformation.customFieldValues && recurrentInformation.customFieldValues.length > 0) {
            body.customFieldValues = recurrentInformation.customFieldValues;
        }
        return new Promise(function (resolve) { return __awaiter(_this, void 0, void 0, function () {
            var response, error_6;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 2, , 3]);
                        return [4 /*yield*/, axios_1.default.post(this.url + "/rest/processTx/RECURRENT", body, { headers: this.header })];
                    case 1:
                        response = _a.sent();
                        resolve(response.data);
                        return [3 /*break*/, 3];
                    case 2:
                        error_6 = _a.sent();
                        if (error_6)
                            throw new Error(error_6);
                        return [3 /*break*/, 3];
                    case 3: return [2 /*return*/];
                }
            });
        }); });
    };
    return PagueloFacil;
}());
exports.default = PagueloFacil;
